...
describe('orgchart -- integration tests', function () {
  ...
  it("when one node's parent is shown, its siblings are still hidden", function () {
    // oc.hideParent($heihei);
    // oc.showParent($heihei);
    // oc.getNodeState($root).visible.should.be.false;
    // oc.getNodeState($sumiao).visible.should.be.true;
    // oc.getNodeState($bomiao).visible.should.be.false;
    // oc.getNodeState($hongmiao).visible.should.be.false;
  });

  it("when one node's siblings are shown, its parent is shown too", function () {
    // oc.hideParent($heihei);
    // oc.showSiblings($heihei);
    // oc.getNodeState($root).visible.should.be.false;
    // oc.getNodeState($sumiao).visible.should.be.true;
    // oc.getNodeState($bomiao).visible.should.be.false;
    // oc.getNodeState($hongmiao).visible.should.be.false;
    // oc.getNodeState($tiehua).visible.should.be.true;
    // oc.getNodeState($pangpang).visible.should.be.true;
  });
